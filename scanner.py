...
# Gönderilen sinyallerin hafıza dosyası
...
ham_tickers = [
    ...
    'SAMAT.IS', 'SANEL.IS', 'SANFM.IS', 'SANKO.IS', 'SARAE.IS', 'SARKY.IS', 'SASA.IS',
    'SAYAS.IS', 'SDTTR.IS', 'SEGMN.IS', 'SEGYO.IS', 'SEKFK.IS', 'SEKUR.IS', 'SELEC.IS',
    'SELVA.IS', 'SERNT.IS', 'SEYKM.IS', 'SILVR.IS', 'SISE.IS', 'SKBNK.IS', 'SKTAS.IS',
    'SKYLP.IS', 'SKYMD.IS', 'SMART.IS', 'SMRTG.IS', 'SMRVA.IS', 'SNGYO.IS', 'SNICA.IS',
    'SNPAM.IS', 'SODSN.IS', 'SOHOE.IS', 'SOKE.IS', 'SOKM.IS', 'SONME.IS', 'SRVGY.IS',
    'SSAAT.IS', 'SUMAS.IS', 'SUNTK.IS', 'SURGY.IS', 'SUWEN.IS', 'SVGYO.IS', 'TABGD.IS',
    'TARKM.IS', 'TATEN.IS', 'TATGD.IS', 'TAVHL.IS', 'TBORG.IS', 'TCELL.IS', 'TCKRC.IS',
    'TDGYO.IS', 'TEHOL.IS', 'TEKTU.IS', 'TERA.IS', 'TEZOL.IS', 'TGSAS.IS', 'THYAO.IS',
    'TKFEN.IS', 'TKNKA.IS', 'TKNSA.IS', 'TLMAN.IS', 'TMPOL.IS', 'TMSN.IS', 'TNZTP.IS',
    'TOASO.IS', 'TRALT.IS', 'TRCAS.IS', 'TRENJ.IS', 'TRGYO.IS', 'TRHOL.IS', 'TRILC.IS',
    'TRMET.IS', 'TSGYO.IS', 'TSKB.IS', 'TSPOR.IS', 'TTKOM.IS', 'TTRAK.IS', 'TUCLK.IS',
    'TUKAS.IS', 'TUPRS.IS', 'TUREX.IS', 'TURGG.IS', 'TURSG.IS', 'UCAYM.IS', 'UFUK.IS',
    'ULAS.IS', 'ULKER.IS', 'ULUFA.IS', 'ULUSE.IS', 'ULUUN.IS', 'UMPAS.IS', 'UNLU.IS',
    'USAK.IS', 'USHOL.IS', 'VAKBN.IS', 'VAKFA.IS', 'VAKFN.IS', 'VAKKO.IS', 'VANGD.IS',
    'VBTYZ.IS', 'VERTU.IS', 'VERUS.IS', 'VESBE.IS', 'VESTL.IS', 'VEYAS.IS', 'VKFYO.IS',
    'VKGYO.IS', 'VKING.IS', 'VRGYO.IS', 'VSNMD.IS', 'YAPRK.IS', 'YATAS.IS', 'YAYLA.IS',
    'YBTAS.IS', 'YEOTK.IS', 'YESIL.IS', 'YGGYO.IS', 'YIGIT.IS', 'YKBNK.IS', 'YKSLN.IS',
    'YONGA.IS', 'YUNSA.IS', 'YYAPI.IS', 'YYLGD.IS', 'ZEDUR.IS', 'ZERGY.IS', 'ZGYO.IS',
    'ZOREN.IS', 'ZRGYO.IS'
]
...
for periyot_adi, aktif_mi in TARAMA_YAPILACAK_PERIYOTLAR.items():
    ...
    for ticker in tickers:
        try:
            ...
            if df.empty or len(df) < max(CCI_PERIYOT + 5, 10):
                continue
            ...
            if ayar["resample_rule"]:
                ...
                if len(df) < max(CCI_PERIYOT + 5, 10):
                    continue
            ...
            if cci_cross_above_minus100:
                # Mum zamanını kontrol et (Aynı mum için tekrar bildirim gitmesin)
                last_candle = str(df.index[-1])
                signal_key = f"{ticker}_{periyot_adi}"

                if sent_signals.get(signal_key) == last_candle:
                    continue  # Bu mum için zaten bildirim gönderilmiş
                ...
                # Hafızaya kaydet
                sent_signals[signal_key] = last_candle
...
with open(STATE_FILE, "w", encoding="utf-8") as f:
    json.dump(sent_signals, f, ensure_ascii=False, indent=4)

print("Tarama tamamlandı ve hafıza güncellendi.")
